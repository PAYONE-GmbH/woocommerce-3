...
class PayDirekt extends GatewayBase {
	...
	public function process_payment( $order_id ) {
		...
		if ( $this->is_redirect( 'success' ) ) {
			...
			$make_redirect = true;
			$is_success = false;
			wc_add_notice( __( 'Payment error: ', 'payone-woocommerce-3' ) . __( 'PayDirekt returned error',
			...
			$transaction = new \Payone\Transaction\PayDirekt( $this );
	...
	private function handle_successfull_payment( \WC_Order $order ) {
		...
		if ( $authorization_method === 'preauthorization' ) {
			$order->update_status( 'on-hold', __( 'PayDirekt payment is preauthorized.', 'woocommerce' ) );
		} elseif ( $authorization_method === 'authorization' ) {
			$order->add_order_note( __( 'PayDirekt payment is authorized and captured.', 'woocommerce' ) );
